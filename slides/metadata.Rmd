...
We've chosen the "CCBY" license and the markdown file type (`.md`).  Markdown files are plain text files that are easily edited in RStudio.  Try opening the `abstract.md` file (in the `./storm_project/metadata_templates/` folder) and adding a brief abstract.  
{:.notes}
...
If you type `issues()` in your R console, you'll get helpful info on which issues you need to fix, such as: 
...
Many repositories for environmental data use EML (Ecological Metadata Language) formatting for metadata.  The function `make_eml()` converts the text files we've been editing into EML documents.

```{r, eval = FALSE, message = FALSE, handout = 0}
make_eml(eml_doc, "storm_project/metadata/?????????????")
```
